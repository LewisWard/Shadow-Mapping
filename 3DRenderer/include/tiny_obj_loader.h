...
//
// I have modified this file from the original, you can download the origin from:
...
public:
  ...
  virtual bool operator()(const std::string &matId, std::vector<material> &materials, std::map<std::string, int> &matMap, std::string &err) = 0;
};
...
static bool exportFaceGroupToShape(
    shape &shape, std::map<vertex_index, unsigned int> vertexCache,
    const std::vector<float> &in_positions,
    const std::vector<float> &in_normals,
...
void LoadMtl(std::map<std::string, int> &material_map,
             std::vector<material> &materials, std::istream &inStream) {
...
bool LoadObj(std::vector<shape> &shapes,       // [output]
             std::vector<material> &materials, // [output]
  ...
  while (inStream.peek() != -1) {
    ...
    if ((0 == strncmp(token, "usemtl", 6)) && IS_SPACE((token[6]))) {
      ...
      if (newMaterialId != material) {
        ...
        exportFaceGroupToShape(aShape, vertexCache, v, vn, vt, faceGroup, tags,
  ...
  bool ret = exportFaceGroupToShape(aShape, vertexCache, v, vn, vt, faceGroup,
